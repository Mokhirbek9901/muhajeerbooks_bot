...
new_image = '''def _sync_app_images_to_telegram(rows, previous_rows):
    previous = {
        str(row.get("id")): row
        ...
        try:
            if needs_photo:
                ...
                if new_id:
                    _rpc(
                        "bot_sync_set_photo",
                        {"p_secret": SYNC_SECRET, "p_cloud_id": cloud_id, "p_photo_id": new_id},
                    )
                ...
                _rpc(
                    "bot_sync_set_photo",
                    {"p_secret": SYNC_SECRET, "p_cloud_id": cloud_id, "p_photo_id": ""},
                )
...
new_loop = '''def sync_loop():
    ...
    while True:
        try:
            # BOOKS — Supabase markaziy katalog; botdagi haqiqiy o‘zgarishlar ikki tomonga o'tadi.
            local = _read_books()
            ...
            if not initialized:
                ...
                if changed_books:
                    _push_books(changed_books)

            rows = _pull_books()

            # Appdagi rasm qo‘shish/almashtirish/o‘chirish Telegramga ham o'tadi.
            ...
            # Network kutish vaqtida bot katalogi o‘zgargan bo‘lsa ustidan yozib yubormaymiz.
            ...
            # ORDERS — bot va programma bitta Supabase orders bazasidan foydalanadi.
            ...
            for key, order in list(local_orders.items()):
                ...
                if str(order.get("source") or "telegram") == "app":
                    continue
                try:
                    # Legacy bot orderlar uchun create RPC cart/items formatini xavfsiz normalizatsiya qiladi.
                    result = _rpc(
                        "bot_order_create",
            ...
            for row in cloud_orders:
                ...
                try:
                    ...
                    continue
                item = _cloud_order_to_bot(row, books_by_uuid, latest_orders.get(key))
                if item is None:
                    continue
...
# Obsolete helper names must never remain in production code.
for obsolete in ('_sync_telegram_covers(', '_merge_cloud(', '_pull_rows('):
    assert obsolete not in s, obsolete
assert '_sync_app_images_to_telegram(rows, last_cloud_rows)' in s
assert 'bot_sync_delete' in s
assert 'bot_order_create' in s

p.write_text(s, encoding='utf-8')
print('final live sync patch applied')
